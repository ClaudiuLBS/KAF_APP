...
class Consumer(models.Model):
    ...
    referral_code = models.CharField(max_length=6, unique=True, default='nimic')
    ...
    adus_de = models.ForeignKey('self', on_delete=models.SET_NULL, blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.adus_de == None:
            try:
                x = Consumer.objects.filter(referral_code__iexact=self.friend_refferal)
                self.adus_de = x[0]
                z = x[0]
                z.friends += 1
                z.save()
        ...
        if self.referral_code == 'nimic':
            code = get_random_string(length=6, allowed_chars='QWERTYUIOPASDFGHJKLZXCVBNM')
